...
resource "null_resource" "apply" {
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "ansible-pull -i /tmp/hosts -U https://github.com/kesavakadiyala/ansible.git roboshop_project.yml -t ${var.component} -e PAT=${nonsensitive(jsondecode(data.aws_secretsmanager_secret_version.creds.secret_string)["PAT"])} -e ENV=${var.ENV} -e CATALOGUE_PORT=80 -e CART_PORT=80 -e USER_PORT=80"
    ]
  }
}
